 ...
 */
final class Version20200506185927 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__category AS SELECT id, name_id, parent_category FROM category');
        ...
        $this->addSql('INSERT INTO category (id, name_id, parent_category) SELECT id, name_id, parent_category FROM __temp__category');
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__unit AS SELECT id, unit FROM unit');
        $this->addSql('DROP TABLE unit');
        $this->addSql('CREATE TABLE unit (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, full_name_id INTEGER DEFAULT NULL, unit VARCHAR(10) NOT NULL COLLATE BINARY, CONSTRAINT FK_DCBB0C53223EC5BA FOREIGN KEY (full_name_id) REFERENCES text (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO unit (id, unit) SELECT id, unit FROM __temp__unit');
        $this->addSql('DROP TABLE __temp__unit');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_DCBB0C53223EC5BA ON unit (full_name_id)');
    }
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__category AS SELECT id, name_id, parent_category FROM category');
        $this->addSql('DROP TABLE category');
        $this->addSql('CREATE TABLE category (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name_id INTEGER NOT NULL, parent_category INTEGER DEFAULT NULL)');
        $this->addSql('INSERT INTO category (id, name_id, parent_category) SELECT id, name_id, parent_category FROM __temp__category');
        $this->addSql('DROP TABLE __temp__category');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_64C19C171179CD6 ON category (name_id)');
        $this->addSql('DROP INDEX IDX_D34A04AD12469DE2');
        ...
        $this->addSql('CREATE INDEX IDX_B469456F698D3548 ON translation (text_id)');
        $this->addSql('CREATE INDEX IDX_B469456F82F1BAF4 ON translation (language_id)');
        $this->addSql('DROP INDEX UNIQ_DCBB0C53223EC5BA');
        $this->addSql('CREATE TEMPORARY TABLE __temp__unit AS SELECT id, unit FROM unit');
        $this->addSql('DROP TABLE unit');
        $this->addSql('CREATE TABLE unit (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, unit VARCHAR(10) NOT NULL)');
        $this->addSql('INSERT INTO unit (id, unit) SELECT id, unit FROM __temp__unit');
        $this->addSql('DROP TABLE __temp__unit');
    }
}
